...
public class BMLADIS16470_IMU implements AutoCloseable, NTSendable {
  ...
  private static final double rad_to_deg = 5.729577951308232e1; // 180/PI
  private static final double deg_to_rad = 1/rad_to_deg;
  private static final double grav = 9.80615;
  ...
  private static class AcquireTask implements Runnable {
    private BMLADIS16470_IMU imu;

    public AcquireTask(BMLADIS16470_IMU imu) {
      this.imu = imu;
    }
  ...
  public BMLADIS16470_IMU() {
    this(IMUAxis.kY, SPI.Port.kOnboardCS0, CalibrationTime._4s);
  }
   ...
   */
  public BMLADIS16470_IMU(IMUAxis yaw_axis, SPI.Port port, CalibrationTime cal_time) {
    m_yaw_axis = yaw_axis;
    ...
    m_simDevice = SimDevice.create("Gyro:BMLADIS16470", port.value);
    if (m_simDevice != null) {
  ...
  private boolean switchToStandardSPI() {
    ...
    if (m_spi == null) {
      ...
      if (readRegister(PROD_ID) != 16470) {   // don't know why this was '16982' in the original driver -bb
        DriverStation.reportError("Could not find ADIS16470", false);
        DriverStation.reportError("BML Found PROD_ID:"+ Integer.toString(PROD_ID), false);
        close();
        return false;
      }
      return true;
  ...
  public void calibrate() {
    ...
    ;
  } // OK, maybe I missed something, but there seems to be NO calibration code here!!!
  ...
  /** - bb
   * @return X-axis Unfiltered X GyroRate
   */
